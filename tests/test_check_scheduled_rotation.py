...
class TestCheckScheduledRotation:
    ...
    def test_rotation_needed_exactly_14_days(self):
        ...
        last_date = (datetime.now() - timedelta(days=14)).strftime("%d-%m-%Y")
    ...
    def test_rotation_needed_more_than_14_days(self):
        ...
        last_date = (datetime.now() - timedelta(days=21)).strftime("%d-%m-%Y")
        exit_code, stdout, stderr = run_check_script(last_date)
    ...
    def test_rotation_not_needed_less_than_14_days(self):
        ...
        last_date = (datetime.now() - timedelta(days=7)).strftime("%d-%m-%Y")
        exit_code, stdout, stderr = run_check_script(last_date)
    ...
    def test_rotation_not_needed_13_days(self):
        ...
        last_date = (datetime.now() - timedelta(days=13)).strftime("%d-%m-%Y")
        exit_code, stdout, stderr = run_check_script(last_date)
    ...
    def test_invalid_date_format_treated_as_first_run(self):
        ...
        assert "Expected format: DD-MM-YYYY" in stdout
        assert "Treating as first run" in stdout

    def test_invalid_date_format_yyyy_mm_dd(self):
        """Test when date format is YYYY-MM-DD instead of DD-MM-YYYY"""
        exit_code, stdout, stderr = run_check_script("2025-10-29")

        assert exit_code == 0, "Wrong format should return exit code 0 (rotation needed)"
        assert "Invalid date format" in stdout
        assert "2025-10-29" in stdout

    def test_output_includes_last_date(self):
        """Test that output includes the last rotation date"""
        last_date = "15-10-2025"
        exit_code, stdout, stderr = run_check_script(last_date)

        assert "Last scheduled rotation: 15-10-2025" in stdout
    ...
    def test_very_old_rotation(self):
        ...
        last_date = (datetime.now() - timedelta(days=365)).strftime("%d-%m-%Y")
        exit_code, stdout, stderr = run_check_script(last_date)
...
class TestCheckScheduledRotationIntegration:
    ...
    def test_edge_case_leap_year(self):
        ...
        # Should be > 14 days from 28-02-2024 to now (2025-10-29)
