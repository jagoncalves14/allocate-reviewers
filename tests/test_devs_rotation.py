...
def test_allocate_reviewers_extreme_minimal_scenario() -> None:
    """
    Minimal scenario stress test with retry mechanism.
...
@patch("lib.env_constants.EXPERIENCED_DEV_NAMES", {
    "Dev1", "Dev2", "Dev3", "Dev4", "Dev5", "Dev6",
    "Dev7", "Dev8", "Dev9", "Dev10", "Dev11", "Dev12"
})
def test_non_experienced_devs_are_assigned() -> None:
    """
    Test that non-experienced developers are assigned as reviewers.
    ...
    - Experienced devs can have at most 1 non-experienced reviewer
    """
    # 12 experienced, 2 non-experienced (NonExperiencedDev1 and NonExperiencedDev2)
    developers = [
...
@patch("lib.env_constants.EXPERIENCED_DEV_NAMES", {
    "ExpA", "ExpB", "ExpC", "ExpD", "ExpE", "ExpF"
})
def test_non_experienced_devs_load_balancing() -> None:
    """
    Test that non-experienced developers are distributed fairly
    when being assigned as reviewers (load balancing).
    
    6 experienced devs, 2 non-experienced devs
...
@patch("lib.env_constants.EXPERIENCED_DEV_NAMES", {"ExpA", "ExpB", "ExpC"})
def test_non_experienced_with_insufficient_slots() -> None:
    """
    Test with tight constraints but still possible.
    
    3 experienced devs, each needs 2 reviewers.
...
def test_non_experienced_assignment_multiple_runs() -> None:
    ...
    consistently assigned (not just by luck).
    """
